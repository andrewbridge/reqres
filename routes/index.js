...
function returnAll(items, req, res) {
	...
	var page = parseInt(req.query.page, 10) || 1,
		...
		filters = Object.keys(req.query).filter(key => key.indexOf(filterByPreffix) !== -1).map(key => key.replace(new RegExp(`^${filterByPreffix}`), '')).filter(key => key.length),
		filteredItems = items.reduce((itemPrevious, itemCurrent) => itemPrevious.concat(filters.reduce((filterPrevious, filterCurrent) => !filterPrevious ? false : !(typeof itemCurrent[filterCurrent] !== 'string' || itemCurrent[filterCurrent].indexOf(query[`${filterByPreffix}${filterCurrent}`]) === -1), true) ? [itemCurrent] : []), []),
